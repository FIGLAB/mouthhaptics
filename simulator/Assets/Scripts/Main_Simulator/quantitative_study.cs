...
public class quantitative_study : MonoBehaviour
{
    ...
    // variables for spatial sensitivity
    public float outerLip = 0.015f;
    public float innerLip = 0.0f;
    public double spatialRes = 0.0015;
    private Vector3 spatialResVec;
    private Vector3 outerPos;
    private Vector3 innerPos;
    private double outerDist = 0.03;
    private int outerCount = 0;
    private double innerDist = 0.0;
    private int innerCount = 0;
    private bool spaceTest = false;
    private bool isOuter = true;

    public bool innerDone = false;
    public bool outerDone = false;
    ...
    void Start()
    {
        ...
        spatialResVec = new Vector3((float)spatialRes, 0.0f, 0.0f);
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown("space"))
        {
            if (spaceTest)
            {
                if (isOuter)
                {
                    ...
                    Thread.Sleep(500);
                    tapAnim(outerPos);
                    Debug.Log("Outer test current dist: " + outerDist);
                }
                else {
                    tapAnim(innerPos);
                    Thread.Sleep(500);
                    tapAnim(this.transform.position);
                    Debug.Log("Inner test current dist: " + innerDist);
                }
            }
            else
            {
        ...
        if (Input.GetKeyDown("y"))
        {
            if (isOuter) {
                outerCount += 1;
                if (outerCount == 2) { outerCount = 0; outerPos -= spatialResVec; outerDist -= spatialRes; }
                if (!innerDone) { isOuter = false; }
            }
            else
            {
                innerCount += 1;
                if (innerCount == 2) { innerCount = 0; innerPos -= spatialResVec; innerDist -= spatialRes; }
                if (!outerDone) { isOuter = true; }
            }
        } else if (Input.GetKeyDown("n"))
        {
            if (isOuter) { outerPos += spatialResVec; outerDist += spatialRes;
                if (!innerDone) { isOuter = false; }
            }
            else { innerPos += spatialResVec; innerDist += spatialRes;
                if (!outerDone) { isOuter = true; }
            }
        }
        ...
        {
            Debug.Log("SPATIAL RESOLUTION RESET!!!");
            outerPos = this.transform.position + new Vector3(outerLip, 0.0f, 0.0f);
            innerPos = this.transform.position + new Vector3(innerLip, 0.0f, 0.0f) ;
            spaceTest = true;
            isOuter = true;
            currentHz = 50;
            outerDist = outerLip*2;
            innerDist = innerLip*2;
        }

        if (Input.GetKeyDown("f"))
        {
            Debug.Log("FREQUENCY TEST RESET!!!");
            spaceTest = false;
            isOuter = true;
            currentHz = 50;
        }
