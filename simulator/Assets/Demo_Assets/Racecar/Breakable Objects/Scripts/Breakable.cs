...
namespace WSMGameStudio.Behaviours
{
    public class Breakable : MonoBehaviour
    {
        ...
        public void Break()
        {
            if (anims.animDone) { StartCoroutine(anims.oneTap(lips.transform.position, ms_delay: 700, hz: 90)); }
            if (_breakSFX != null)
                _breakSFX.Play();
            ...
            foreach (var piece in brokenPieces)
            {
                ...
                SphereCollider collider = pieceClone.AddComponent(typeof(SphereCollider)) as SphereCollider;
